...
def news_detail(news_id):
    ...
    try:
        ...
        print(news,news.id,news.title,news.comments)
        # print(news.to_dict())
    ...
    # print(news)
    click_news_list = []
    for temp in news_list if news_list else []:
        # print(news,news.clicks)
        click_news_list.append(temp.to_basic_dict())
    # 获取当前新闻的评论
    # print(click_news_list,news_list)
    comments = []
    # print(news)
    try:
    ...
    if comments:
        for item in comments if comments else []:
            comment_dic=item.to_dict()
            comment_dic['is_like']=False
            comment_like=None
            try:
                comment_like=CommentLike.query.filter(CommentLike.comment_id==item.id,CommentLike.user_id==g.user.id).first()
            except Exception as e:
                current_app.logger.error(e)
            if comment_like:
    ...
    if g.user:
        if news in g.user.collection_news:
            is_collected = True
        if news.user:
            if news.user.followers.filter(User.id == g.user.id).first():
                is_followed=True
    # print(news.to_dict())
    data = {
        "news": news.to_dict(),
...
def followed_user():
    ...
    # 根据不同操作做不同逻辑
    if action == "follow":
        if target_user.followers.filter(User.id == g.user.id).first() :
