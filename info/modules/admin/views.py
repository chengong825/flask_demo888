...
from flask import session, render_template, current_app, request, g, redirect, url_for, jsonify
import time
...
def admin_login():
    ...
    # session[str(user.id)] = {"user_id": user.id, "nick_name": user.nick_name, "mobile": user.mobile,"is_admin":True}
...
def user_count():
    ...
    # print(now_date,datetime.now().strftime('%Y-%m-%d'),day_begin_date)
    # print(now_date==day_begin_date)
    # print(datetime.now(),mon_begin_date)
    now_date=day_begin_date
    # 定义空数组，保存数据
    active_date = []
    active_count = []
...
def news_rank():
    ...
    return render_template('admin/news_edit_clickslist.html', data=context)
