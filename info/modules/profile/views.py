from flask import render_template, redirect, g, request, jsonify, current_app, session, abort
...
def news_release():
    ...
    # 1.1 判断数据是否有值
    print('title:{}, source:{}, digest:{}, content:{}, index_image:{}, category_id:{}'.format(title, source, digest, content, index_image, category_id))
    print(all([title, source, digest, content, index_image, category_id]))
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.PARAMERR, errmsg="无法读取图片")
...
def other_info():
    ...
    is_followed = False
    if g.user:
        if other.followers.filter(User.id == user.id).first():
            is_followed = True
